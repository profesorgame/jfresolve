...
namespace Jfresolve.Utilities
{
    ...
    /// Consolidates URL building logic that was scattered across multiple files.
    /// Extracted from: JfresolveApiController.cs, StrmFileGenerator.cs, InsertActionFilter.cs, JfresolvePopulator.cs
    /// </summary>
    public static class UrlBuilder
    {
        ...
        /// <summary>
        /// Builds a season folder path within a series folder.
        /// </summary>
        public static string BuildSeasonPath(string seriesPath, int seasonNumber)
        {
            if (string.IsNullOrWhiteSpace(seriesPath))
                throw new ArgumentException("Series path cannot be empty", nameof(seriesPath));

            var seasonFolderName = FileNameUtility.BuildSeasonFolderName(seasonNumber);
            return System.IO.Path.Combine(seriesPath, seasonFolderName);
        }
    }
}
